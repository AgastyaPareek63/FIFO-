...
module async_fifo #(
    ...
    parameter ECC_WIDTH = DATA_WIDTH + PARITY_BITS + 1,// data + parity + overall parity
    ...
    // Binary pointers
    reg [ADDR_WIDTH:0] wr_bin;// write pointer in binary
    reg [ADDR_WIDTH:0] rd_bin;// read pointer in binary

    // Gray-code pointers
    reg [ADDR_WIDTH:0] wr_gray;// write pointer in Gray code
    reg [ADDR_WIDTH:0] rd_gray;// read pointer in Gray code
    ...
    // Synchronized pointers from the opposite clock domain
    ...
    // ECC data path
    wire [ECC_WIDTH-1:0] mem_wr_data;// ECC encoded data written to memory
    wire [ECC_WIDTH-1:0] mem_rd_data;// ECC encoded data read from memory

    wire [DATA_WIDTH-1:0] decoded_data;// decoded FIFO data
    ...
    // FIFO occupancy
    // Write side uses synchronized read pointer
    // Read side uses synchronized write pointer
    assign wr_used = wr_bin - rd_bin_sync;
    assign rd_used = wr_bin_sync - rd_bin;


    // Next write and read pointer values
...
// Full when the next write pointer reaches the read pointer with the wrap bits inverted.
            full_reg <=
                (wr_gray_next ==
                {
...
// Empty when the next read pointer reaches the synchronized write pointer
            empty_reg <= (wr_gray_sync == rd_gray_next);
    ...
    // ECC encoder
    // Data is encoded before being stored in FIFO memory.
    ecc_encoder #(
    ...
    // FIFO memory
    // Memory stores the ECC encoded data.
    fifo_memory #(
    ...
    // ECC decoder
    // decodes the data after it is read from FIFO memory.
